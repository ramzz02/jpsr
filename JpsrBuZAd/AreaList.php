...
<?php
  ...
  date_default_timezone_set('Asia/Kolkata');
    ...
    if(isset($_SESSION['area_updated'])){
    if($_SESSION['area_updated']){
      $output = "Successfully updated";
    } else {
      $output = "Not yet updated";
    }
    unset($_SESSION['area_updated']);
    }

    if(isset($_SESSION['area_exists'])){
    if($_SESSION['area_exists']){
      $output = "Update failed. Area already added for particular ward";
    }
    unset($_SESSION['area_exists']);
    }

  if(isset($_SESSION['area_deleted'])) {
    if($_SESSION['area_deleted']) {
        $output = "Successfully deleted";
    } else {
        $output = "Failed to delete";
    }
    unset($_SESSION['area_deleted']);
  }
  ...
  if(isset($_POST['delete_id'])){
    ...
    $deletefile = $deletefile->removeArea($data);
    $deletefile_id = $deletefile->rowCount();
  
    if($deletefile_id){ 
        $_SESSION['area_deleted'] = true;
    } else {
        $_SESSION['area_deleted'] = false;
    }
...
$getareas = $getareas->fetchArea("ORDER BY id DESC")->resultSet();
        ...
        <main class="main-wrapper clearfix">
            ...
            <div class="row page-title clearfix">
                <div class="page-title-left">
                    <h6 class="page-title-heading mr-0 mr-r-5">Area List</h6>
                </div>
                <!-- /.page-title-left -->
                                ...
                                <table class="table table-editable table-responsive" data-toggle="datatables" >
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            $getlocations = $getlocations->fetchLocation("WHERE id = '{$getarea['location']}' ORDER BY id DESC")->resultSet();
                                            $getlocation = $getlocations[0];

                                            $getwards = new Location();
                                            $getwards = $getwards->fetchWard("WHERE id = '{$getarea['ward_no']}' ORDER BY id DESC")->resultSet();
                                            $getward = $getwards[0];
                                            ?>
                                            <td><?php echo $getlocation['location_name']; ?></td>
                                            <td><?php echo $getward['ward_no']; ?></td>
                                            <td><?php echo $getarea['area_name']; ?></td>
                                            <td>
                                            <label class="switch">
                                            <input class="switch-input" type="checkbox" name="checkbox" <?php if(isset($getarea['status'])){ if($getarea['status'] == '1'){ echo 'checked'; } } ?> />
                                            <span class="switch-label active_update" id="<?php echo $getarea['id'];?>" data-on="Active" data-off="Deactive"></span>
                                            <span class="switch-handle "></span> 
                                           </label>
                                            </td>
                                            <td style="text-align: center;">
                                                <a href="JavaScript:Void(0);" id="<?php echo $getarea['id'];?>" class="edit">
...
<script type="text/javascript">
  ...
  $(document).on('click', '.delete', function() {   
      ...
      if (confirm_msg == true) {
          ...
          $('#tableForm').attr('method', 'post');
          $('#tableForm').attr('action', 'AreaList');
          $('#tableForm').submit();
      }            
  });
  $(document).on('click', '.active_update', function() {   
          ...
          $.ajax({
            ...
            data: "update_id="+profile_id+"&name=area",
