...
<?php
  ...
  if(isset($_POST['delete_id'])){
    ...
    $deletefile = $deletefile->removeBus($data);
...
$getbuslists = new Business();
$getbuslists = $getbuslists->fetchBus("ORDER BY id DESC")->resultSet();
        ...
        <main class="main-wrapper clearfix">
            ...
            <div class="row page-title clearfix">
                <div class="page-title-left">
                    <h6 class="page-title-heading mr-0 mr-r-5">Bus Timings List</h6>
                </div>
                <!-- /.page-title-left -->
                        ...
                        <div class="widget-bg">
                            ...
                            <div class="widget-body clearfix table-list">
                                ...
                                <table class="table table-editable table-responsive" data-toggle="datatables" >
                                    ...
                                    <tbody>
                                        <?php
                                        if(count($getbuslists)){
                                            $count = 1;
                                        foreach($getbuslists as $getbuslist){
                                        ?>
                                        <tr>
                                            <td style="text-align: center;"><?php echo $count; ?></td>
                                            <td><?php echo ucwords($getbuslist['bus_type']); ?></td>
                                            <td><?php echo $getbuslist['bus_name']; ?></td>
                                            <td><?php echo $getbuslist['days']; ?></td>
                                            <td><?php echo $getbuslist['departure_place'].' & '.$getbuslist['departure_time']; ?></td>
                                            <td><?php echo $getbuslist['arrival_place'].' & '.$getbuslist['arrival_time']; ?></td>
                                            <td style="text-align: center;">
                                                <a href="JavaScript:Void(0);" id="<?php echo $getbuslist['id'];?>" class="edit">
                                                  <i class="feather feather-edit fs-20" style="color: #3a7cdc;font-weight: 700;"></i>
                                                </a>&nbsp;
                                                <a href="JavaScript:Void(0);" id="<?php echo $getbuslist['id'];?>" class="delete">
...
<script type="text/javascript">
  $(document).on('click', '.edit', function() {                  
        ...
        $('#tableForm').attr('action', 'Bus-Timings');
        $('#tableForm').submit();
    });
  $(document).on('click', '.delete', function() {   
      ...
      if (confirm_msg == true) {
          ...
          $('#tableForm').attr('method', 'post');
          $('#tableForm').attr('action', 'Bus-Timings-List');
          $('#tableForm').submit();
      }            
  });
</script>
