...
<?php
  ...
  if(isset($_SESSION['business_added'])){
    ...
    if(isset($_POST['productSubmit'])){
        ...
        if($_POST['logo_subscription_type'] == ''){
         $logo_subs_order = 0;
        } else {
         $logo_subs_order = 1;   
        }

        if($_POST['gallery_subscription_type'] == ''){
         $gallery_subs_order = 0;
        } else {
         $gallery_subs_order = 1;   
        }

        if($_POST['video_subscription_type'] == ''){
         $video_subs_order = 0;
        } else {
         $video_subs_order = 1;   
        }
        ...
        if($_POST['logo_subscription_type'] == '' && $_POST['gallery_subscription_type'] == '' && $_POST['video_subscription_type'] == ''){
        $free_subs_order = 1;
        }

        $merge_subs_order = $logo_subs_order + $gallery_subs_order + $video_subs_order + $free_subs_order;

        $data[] = $_POST['payment_type'];
        $data[] = $merge_subs_order;
        $data[] = date("Y/m/d");
        $data[] = "admin";
        ...
        if($_FILES['logo_image']['name']){
          ...
          if($_POST['logo_subscription_type'] != ''){
            ...
            $imgsubs[] = $_POST['logo_subscription_type'];

            $getlogoperiods = new Business();
            $getlogoperiods = $getlogoperiods->fetchPeriod("WHERE period = '{$_POST['logo_subscription_type']}' ORDER BY id ASC")->resultSet();
            $getlogopes = $getlogoperiods[0];

            $getlogodefaulsubs = new Business();
            $getlogodefaulsubs = $getlogodefaulsubs->fetchSubscription("WHERE type = '1' AND period = '{$getlogopes['id']}' ORDER BY id ASC")->resultSet();
            $getlogodefaulsub = $getlogodefaulsubs[0];
            $imgsubs[] = $getlogodefaulsub['amount'];

            $totalDays = $getlogopes['period_days'].' day';
            ...
            $addbusinesssubscriptionsdta = new Business();
            $addbusinesssubscriptionsdta = $addbusinesssubscriptionsdta->addBusinessPayment($imgsubs);
            $addbusinesssubscriptionsdtaID = $addbusinesssubscriptionsdta->lastInsertID();
        ...
        if($_FILES['video_path']['name']){
          ...
          if($_POST['video_subscription_type'] != ''){
            ...
            $imgsubs[] = $_POST['video_subscription_type'];

            $getlogoperiods = new Business();
            $getlogoperiods = $getlogoperiods->fetchPeriod("WHERE period = '{$_POST['video_subscription_type']}' ORDER BY id ASC")->resultSet();
            $getlogopes = $getlogoperiods[0];

            $getlogodefaulsubs = new Business();
            $getlogodefaulsubs = $getlogodefaulsubs->fetchSubscription("WHERE type = '3' AND period = '{$getlogopes['id']}' ORDER BY id ASC")->resultSet();
            $getlogodefaulsub = $getlogodefaulsubs[0];
            $imgsubs[] = $getlogodefaulsub['amount'];
        ...
        if(count($_FILES['gallery_images']['error']) >= 1) {
          ...
          if($_POST['gallery_subscription_type'] != ''){
            ...
            $imgsubs[] = 2;
            $imgsubs[] = "";
            $imgsubs[] = $_POST['gallery_subscription_type'];

            $getlogoperiods = new Business();
            $getlogoperiods = $getlogoperiods->fetchPeriod("WHERE period = '{$_POST['gallery_subscription_type']}' ORDER BY id ASC")->resultSet();
            $getlogopes = $getlogoperiods[0];

            $getlogodefaulsubs = new Business();
            $getlogodefaulsubs = $getlogodefaulsubs->fetchSubscription("WHERE type = '2' AND period = '{$getlogopes['id']}' ORDER BY id ASC")->resultSet();
            $getlogodefaulsub = $getlogodefaulsubs[0];
            $imgsubs[] = $getlogodefaulsub['amount'];
