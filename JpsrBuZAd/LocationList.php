...
<?php
  ...
  date_default_timezone_set('Asia/Kolkata');
    ...
    if(isset($_SESSION['location_updated'])){
    if($_SESSION['location_updated']){
      $output = "Successfully updated";
    } else {
      $output = "Not yet updated";
    }
    unset($_SESSION['location_updated']);
    }

    if(isset($_SESSION['location_exists'])){
    if($_SESSION['location_exists']){
      $output = "Update failed. Location already added";
    }
    unset($_SESSION['location_exists']);
    }

  if(isset($_SESSION['location_deleted'])) {
    if($_SESSION['location_deleted']) {
        $output = "Successfully deleted";
    } else {
        $output = "Failed to delete";
    }
    unset($_SESSION['location_deleted']);
  }
  ...
  if(isset($_POST['delete_id'])){
    ...
    $deletefile = $deletefile->removeLocation($data);
    $deletefile_id = $deletefile->rowCount();
  
    if($deletefile_id){ 
        $_SESSION['location_deleted'] = true;
    } else {
        $_SESSION['location_deleted'] = false;
    }
                    ...
                    <div class="col-md-12 widget-holder">
                        ...
                        <div class="widget-bg">
                            <div class="widget-heading clearfix">
                                <a href="Addlocation">Add new location</a>
                            </div>
                                ...
                                <table class="table table-editable table-responsive" data-toggle="datatables" >
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                            <label class="switch">
                                            <input class="switch-input" type="checkbox" name="checkbox" <?php if($getlocation['status'] == '1'){ echo 'checked'; } ?> />
                                            <span class="switch-label active_update" id="<?php echo $getlocation['id'];?>" data-on="Active" data-off="Deactive"></span>
                                            <span class="switch-handle "></span> 
                                           </label>
                                            </td>
                                            <td style="text-align: center;">
                                                <a href="JavaScript:Void(0);" id="<?php echo $getlocation['id'];?>" class="edit">
...
<script type="text/javascript">
  $(document).on('click', '.edit', function() {                  
        ...
        $('#tableForm').attr('action', 'Addlocation');
        $('#tableForm').submit();
    });
  $(document).on('click', '.delete', function() {   
      ...
      if (confirm_msg == true) {
          ...
          $('#tableForm').attr('method', 'post');
          $('#tableForm').attr('action', 'LocationList');
          $('#tableForm').submit();
      }            
  });
  $(document).on('click', '.active_update', function() {   
          ...
          $.ajax({
            ...
            data: "update_id="+profile_id+"&name=city_name",
