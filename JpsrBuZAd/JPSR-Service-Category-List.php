...
<?php
  ...
  date_default_timezone_set('Asia/Kolkata');
    ...
    if($_SESSION['category_exists']){
      $output = "Update failed. Category already added for particular services";
    }
  ...
  if(isset($_POST['delete_id'])){
    ...
    $deletefile = $deletefile->removeCategory($data);
    $deletefile_id = $deletefile->rowCount();
  
    if($deletefile_id){ 
        $_SESSION['category_deleted'] = true;
    } else {
        $_SESSION['category_deleted'] = false;
    }

    header('Location: JPSR_Service-Category-List');

  }



$getcategorys = new Service();
$getcategorys = $getcategorys->fetchCategory("ORDER BY id DESC")->resultSet();
        ...
        <main class="main-wrapper clearfix">
            ...
            <div class="row page-title clearfix">
                <div class="page-title-left">
                    <h6 class="page-title-heading mr-0 mr-r-5">JPSR Category List</h6>
                </div>
                <!-- /.page-title-left -->
                                ...
                                <table class="table table-editable table-responsive" data-toggle="datatables" >
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            $getservices = $getservices->fetchService("WHERE id = '{$getcategory['service_id']}' ORDER BY id DESC")->resultSet();
                                            ...
                                            <td>
                                            <label class="switch">
                                            <input class="switch-input" type="checkbox" name="checkbox" <?php if($getcategory['status'] == 'active'){ echo 'checked'; } ?> />
                                            <span class="switch-label active_update" id="<?php echo $getcategory['id'];?>" data-on="Active" data-off="Deactive"></span>
                                            <span class="switch-handle "></span> 
                                           </label>
                                            </td>
                                            <td style="text-align: center;">
                                                <a href="JavaScript:Void(0);" id="<?php echo $getcategory['id'];?>" class="edit">
                                                  <i class="feather feather-edit fs-20" style="color: #3a7cdc;font-weight: 700;"></i>
                                                </a>&nbsp;
                                                <!-- <a href="JavaScript:Void(0);" id="<?php echo $getcategory['id'];?>" class="delete">
...
<script type="text/javascript">
  $(document).on('click', '.edit', function() {                  
        ...
        $('#tableForm').attr('action', 'JPSR-Service-Category');
        $('#tableForm').submit();
    });
  $(document).on('click', '.delete', function() {   
      ...
      if (confirm_msg == true) {
          ...
          $('#tableForm').attr('method', 'post');
          $('#tableForm').attr('action', 'JPSR-Service-Category-List');
          $('#tableForm').submit();
      }            
  });
  $(document).on('click', '.active_update', function() {   
          ...
          $.ajax({
            ...
            data: "update_id="+profile_id+"&name=service_category",
