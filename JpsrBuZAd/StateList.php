...
<?php
  ...
  date_default_timezone_set('Asia/Kolkata');
    ...
    if(isset($_SESSION['state_updated'])){
    if($_SESSION['state_updated']){
      $output = "Successfully updated";
    } else {
      $output = "Not yet updated";
    }
    unset($_SESSION['state_updated']);
    }

    if(isset($_SESSION['state_exists'])){
    if($_SESSION['state_exists']){
      $output = "Update failed. State already added";
    }
    unset($_SESSION['state_exists']);
    }

  if(isset($_SESSION['state_deleted'])) {
    if($_SESSION['state_deleted']) {
        $output = "Successfully deleted";
    } else {
        $output = "Failed to delete";
    }
    unset($_SESSION['state_deleted']);
  }
  ...
  if(isset($_POST['delete_id'])){
    ...
    $deletefile = $deletefile->removeState($data);
    $deletefile_id = $deletefile->rowCount();
  
    if($deletefile_id){ 
        $_SESSION['state_deleted'] = true;
    } else {
        $_SESSION['state_deleted'] = false;
    }
...
$getstates = $getstates->fetchState("ORDER BY id DESC")->resultSet();
                    ...
                    <div class="col-md-12 widget-holder">
                        ...
                        <div class="widget-bg">
                            <div class="widget-heading clearfix">
                                <a href="Addstate">Add new state</a>
                            </div>
                                ...
                                <table class="table table-editable table-responsive" data-toggle="datatables" >
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo $getstate['state_name']; ?></td>
                                            <td>
                                            <label class="switch">
                                            <input class="switch-input" type="checkbox" name="checkbox" <?php if($getstate['status'] == '1'){ echo 'checked'; } ?> />
                                            <span class="switch-label active_update" id="<?php echo $getstate['id'];?>" data-on="Active" data-off="Deactive"></span>
                                            <span class="switch-handle "></span> 
                                           </label>
                                            </td>
                                            <td style="text-align: center;">
                                                <a href="JavaScript:Void(0);" id="<?php echo $getstate['id'];?>" class="edit">
                                                  <i class="feather feather-edit fs-20" style="color: #3a7cdc;font-weight: 700;"></i>
                                                </a>&nbsp;
                                                <!-- <a href="JavaScript:Void(0);" id="<?php echo $getlocation['id'];?>" class="delete">
...
<script type="text/javascript">
  $(document).on('click', '.edit', function() {                  
        ...
        $('#tableForm').attr('action', 'Addstate');
        $('#tableForm').submit();
    });
  $(document).on('click', '.delete', function() {   
      ...
      if (confirm_msg == true) {
          ...
          $('#tableForm').attr('method', 'post');
          $('#tableForm').attr('action', 'StateList');
          $('#tableForm').submit();
      }            
  });
  $(document).on('click', '.active_update', function() {   
          ...
          $.ajax({
            ...
            data: "update_id="+profile_id+"&name=state_name",
