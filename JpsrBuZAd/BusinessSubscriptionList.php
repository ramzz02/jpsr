...
<?php
  ...
  date_default_timezone_set('Asia/Kolkata');
    ...
    if($_SESSION['subs_exists']){
      $output = "Update failed. Subscription already added for particular period";
    }
...
$getsubscriptions = $getsubscriptions->fetchSubscription("ORDER BY id DESC")->resultSet();
        ...
        <main class="main-wrapper clearfix">
            ...
            <div class="row page-title clearfix">
                <div class="page-title-left">
                    <h6 class="page-title-heading mr-0 mr-r-5">Business Subscription List</h6>
                </div>
                <!-- /.page-title-left -->
                    ...
                    <div class="col-md-12 widget-holder">
                        ...
                        <div class="widget-bg">
                            <div class="widget-heading clearfix">
                                <a href="BusinessSubscription">Add new Subscription</a>
                            </div>
                                ...
                                <table class="table table-editable table-responsive" data-toggle="datatables" >
                                    <thead>
                                        <tr>
                                            ...
                                            <th style="width: 10%;">Plan Code</th>
                                            <th>Plan</th>
                                            <th>Amount</th>
                                            <th>Status</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            $getperiods = $getperiods->fetchPeriod("WHERE id = '{$getsubscription['period']}' ORDER BY id DESC")->resultSet();
                                            $getperiod = $getperiods[0];

                                            $getplans = new Business();
                                            $getplans = $getplans->fetchPlan("WHERE id = '{$getsubscription['plan']}' ORDER BY id DESC")->resultSet();
                                            $getplan = $getplans[0];
                                            ?>
                                            <td><?php echo $getperiod['period']; ?></td>
                                            <td><?php echo $getplan['plan_code']; ?></td>
                                            <td><?php echo $getplan['plan_name']; ?></td>
                                            <td><?php echo $getsubscription['amount']; ?></td>
                                            <td>
                                            <label class="switch">
                                            <input class="switch-input" type="checkbox" name="checkbox" <?php if(isset($getsubscription['status'])){ if($getsubscription['status'] == '1'){ echo 'checked'; } } ?> />
                                            <span class="switch-label active_update" id="<?php echo $getsubscription['id'];?>" data-on="Active" data-off="Deactive"></span>
                                            <span class="switch-handle "></span> 
                                           </label>
                                            </td>
                                            <td style="text-align: center;">
                                                <a href="JavaScript:Void(0);" id="<?php echo $getsubscription['id'];?>" class="edit">
...
<script type="text/javascript">
  $(document).on('click', '.edit', function() {                  
        ...
        $('#tableForm').attr('action', 'BusinessSubscription');
        $('#tableForm').submit();
    });
  $(document).on('click', '.delete', function() {   
      ...
      if (confirm_msg == true) {
          ...
          $('#tableForm').attr('method', 'post');
          $('#tableForm').attr('action', 'BusinessSubscriptionList');
          $('#tableForm').submit();
      }            
  });
  $(document).on('click', '.active_update', function() {   
          ...
          $.ajax({
            ...
            data: "update_id="+profile_id+"&name=business_subscription_list",
